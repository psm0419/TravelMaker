	...
	<div class="container">
		...
		<div class="main-content user-detail">
			...
			<form id="editForm" action="/admin/user/update" method="post">
				<input type="hidden" name="userId" value="${user.userId}"> <label>아이디:</label>
				<input type="text" name="userId" value="${user.userId}" disabled><br>

				<label>이름:</label> <input type="text" name="userName"
					value="${user.userName}" disabled><br> <label>닉네임:</label>
				<input type="text" name="nickName" value="${user.nickName}" disabled><br>

				<label>이메일:</label> <input type="email" name="email"
					value="${user.email}" disabled><br> <label>전화번호:</label>
				<input type="tel" name="tel" value="${user.tel}" disabled><br>

				<label>회원 유형:</label> <select name="userType" disabled>
					<option value="ADM" ${user.userType == 'ADM' ? 'selected' : ''}>관리자</option>
					<option value="CUS" ${user.userType == 'CUS' ? 'selected' : ''}>고객</option>
				</select><br> <label>계정 정지 기간:</label>
				<div class="radio-group">
					<input type="radio" name="banned" value="0" checked disabled>
					<label>정상</label> <input type="radio" name="banned" value="7"
						disabled> <label>7일</label> <input type="radio"
						name="banned" value="30" disabled> <label>30일</label> <input
						type="radio" name="banned" value="9999" disabled> <label>영구
						정지</label>
				</div>

				<div class="btn-group">
					<button type="submit" class="btn btn-admin" style="display: none;"
